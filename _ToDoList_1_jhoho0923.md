...
- [x] 240507(화) 오늘 작업한 내용:  
스케줄러 소스코드 개선하여 git저장소에 업데이트 함으로 3차 프로젝트 최종 작업은 마무리 하였습니다.
작업 내역 올리고 마무리 하였음을 알려 드립니다. 이상으로 금일 일지 보고를 마치겠습니다. 모두 개발작업을 하느라 수고 많으셨습니다.


- [x] 240507(수) 오늘 작업한 내용:  
  React 적용된 마지막 소스코드를 chatbot AI적용를 끝낸 부동산 데이터 조회 웹 서비스 페이지를 최종 업데이트 및 배포를 완료하였고(원빈씨가 약 80%이상 작업진행을 완료하였음.), 금일 오후 중 프로젝트 각팀들이 개발한 웹 사이트를 가지고 프로젝트 발표회를 진행할 예정입니다. 모두 고생 많으셨습니다.


- [x] 240507(금) 오늘 작업한 내용:  
  로지스틱 회귀 모델은 두가지중 하나를 예측해준다. (예) 기울기가 0에 가까우면 0으로 예측하고, 1과 가까우면 1로 예측해준다. 

- [x] 240514(월) 오늘 작업한 내용:  
  * 머신러닝의 정의:
 통계모델의 용어 vs 머신러닝 용어
 독립변수: 피처(Feature) 또는 X(전체 Feature), x(개별 feature), input
 종속변수: Label 또는 Target 또는 y
 머신러닝 사용하는데 필요한 대표적인 모듈
  #!pip install scikit-learn
  target = > label 이다. 
  input (data)  => Feature  이다.
  n_neighbors 기본값: 5 
  xxxx => 모델 이름
  xxxxClassifier() => 분류모델
  xxxxRegressor() => 선형회귀 모델 (회귀 모델)
  ordinal 데이터: 순서나 등급이 있는 데이터이다. 
  nominal 데이터: 순서나 등급이 없다.

 라소, 릿지 등 회귀 분석은 기본적으로 모델(수식-그래프)와 데이터(점)사이의 거리를 측정하여 감점하는 방식으로 평가 점수를 매깁니다. 감점을 덜 받는 방식으로 모델의 계수(coefficient)를 조절하는 알고리즘을 써서 학습을 하는데. 과대적합 상황에서는 coefficient값이 괴랄하게 튀는 경우가 생깁니다. 그래서 최대한 적은 수의 계수로, 작은 크기의 계수로  모델을 짜는게 분석하기도 편하고. 곡선의 경계선이 매끈하게 나옵니다. 

 그래서 감점을 매길 때, (점과 그래프 사이의 거리)에 추가적으로
 모델 만드는데 사용했던 계수의 크기를 넣는 거에요.
 계수의 크기를 작게 만드는 것이 합리적인 모델을 만드는 거라는 근거는 확실하지 않은데. 적어도 메끈하고 단순한 형태의 그래프로 모델을 짤 수 있게 합니다. 알파값을 키울수록 계수의 합에 대한 감점이 크게 들어가기 떄문에.
 알파값을 조절함에 따라 그래프의 매끈한 정도를 선택할 수 있어요.

 편의상 곡선이 매끈하다고 표현했는데.
 정확히는 계수가 크게 나올 수록 x값이 작게 편했을 때 예측값이 크게 왔다갔다 하기 떄문에(민감도가 높아짐)  작은 x값 변화에 따라 애가 좀 둔감하게 움직이게 하려고 하는 거라고 생각해도 될거에요.    
...
- [x] 240521(화) 오늘 작업한 내용 
자연어 처리 기초
분류 문제
TF-IDF 이론
분류 문제 해결을 위한 자연어 전처리
분류 모델 작성 및 예측
지도학습 감성분석
기타 응용 예제

이거 며칠 전에 있었던 리엑트 컴퍼런스 주요 변경사항 요약 영상인데. 프론트 관심있는 분들은 참고하세요. 

우리가 배운 버전이 리엑트 18버전이고, 저기 나온 변경사항들이 이전 버전에서 다루기 까다로웠던 것들 개선한거라. 오히려 변경사항 둘러보는 게 지금 배운 버전에 어떤 이슈들이 있는지 파악하기 좋을거에요.

페이스북이 이것저것 오픈소스로 풀어주는건 좋은데, 버전업 때마다 패턴을 싹 다 갈아엎게끔 크게 바꾸는 경향이 있어요. 이번 변경 때는 리엑트 안에서 클라이언트사이드-서버사이드 분리하는 부분. 그전에 useEffect로 처리했던 부분 관련해서 변경사항이 있네요. useTransition인가? 뭘로 바꿨고, 참조관리하는 부분도 변경됐다고 하니. 프론트 면접 준비하실 분들은 저기 나온 키워드들은 알고 있는게 좋을 거에요. 

신입이 경력자보다 나은 순간은 딱 최신기술 더 먼저 알고 있을 때 정도잖아요. (현업으로 근무하면, 어차피 오래된 버전 쓸거고 최신동향 알아봤자 사업굴리는 사람들이 허가 안하면 못쓰는거라. 공부 안하고 철지난 것들만 아는 사람들이 많아요..)

틈틈히 공식문서나 익숙하지 않은 용어들 정리해서 놓는 게 좋을 거에요. 얕게  넓게 아는 게 나쁘건 아닌데, 그래도 한번 배운 걸로 돈받고 일하려면. 배운 것 중 하나라도 확실히 잘 할 줄 알아야 하니까요.
...
5차 평가 안내
본 평가의 목적은 4차 프로젝트를 앞둔 학생 여러분들께  자연어 처리 학습 배운 내용과 프로젝트 기획과 연계하여 평가하기 위함
요구사항
기본 사항
페르소나 분석 수행
프로젝트에서 제공하는 핵심 정보, 서비스를 정의한다.
핵심 정보, 서비스를 사용하는 페르소나를 정의한다.
페르소나의 사용자 행동 변수는 아래 두가지를 고려한다.
가설
웹에서 수집한 자연어와 관련된 연관 단어를 활용
예) ‘축제’ 관련 통합 서비스를 제공하는 웹사이트라면 아래 단계를 거쳐 관련 키워드를 수집한다.
- 네이버 블로그, 뉴스 기사로 ‘축제’ 검색 시 조회되는 글을 수집
- 관련글에서 사용하는 모든 단어를 분석 및 워드 클라우드로 시각화
- 빈도수가 높은 단어를 사용자 행동 변수 대상으로 검토한다.
페르소나 도출
페르소나별 예상 시나리오 도출
...
페르소나 분석을 통해 무엇을 달성하려는지 명확히 정의합니다. 예를 들어, 사용자 경험 개선, 제품 기능 개선, 마케팅 전략 수립 등이 있습니다.
데이터 수집:

다양한 방법을 통해 실제 사용자에 대한 데이터를 수집합니다. 여기에는 인터뷰, 설문조사, 사용자 관찰, 분석 도구 등을 활용할 수 있습니다.
데이터 분석:

수집된 데이터를 분석하여 공통된 특성, 행동 패턴, 목표 등을 파악합니다. 이를 통해 사용자 그룹을 세분화할 수 있습니다.
페르소나 작성:

분석된 데이터를 바탕으로 각 사용자 그룹을 대표하는 페르소나를 작성합니다. 페르소나는 일반적으로 다음과 같은 정보를 포함합니다:
이름, 나이, 직업, 배경 등 기본적인 인구통계학적 정보
목표와 요구사항
행동 패턴과 사용 시나리오
선호도와 고충
기술 수준 및 기타 관련 특성
...
작성된 페르소나를 팀 내에서 공유하고, 제품 개발 및 마케팅 전략에 반영합니다. 페르소나는 의사 결정 과정에서 실제 사용자를 고려하는 데 도움이 됩니다.
페르소나의 예시
예시 1: 전자상거래 웹사이트를 위한 페르소나
이름: 김민수
나이: 34세
직업: 마케팅 매니저
배경: 서울 거주, 결혼, 아이 1명
목표: 빠르고 쉽게 쇼핑하고, 다양한 상품을 비교하여 구매 결정
행동 패턴: 주로 모바일 앱을 통해 쇼핑, 주말에 많이 사용, 리뷰와 평점을 중요하게 생각
고충: 바쁜 일정으로 인해 쇼핑에 많은 시간을 할애할 수 없음, 배송의 신속성과 정확성을 중시
기술 수준: 평균 이상의 기술 숙련도, 최신 기술에 대한 관심이 높음
예시 2: 교육 플랫폼을 위한 페르소나
이름: 정호순
나이: 43세
직업: 개인 사업자, 프리랜서, IT 엔지니어 근무 경험 있음.
배경: 경기도 거주, 미혼
목표: 자영업자 및 영세 소상공인들과 사회적약자들에게 연민을 느껴, 정치가 바뀌어야 한다고 생각하고,
보다 서민들이나 사회적 약자들이 법률 불공정에서 벗어날 수 있게 그에 관련한 법리적 개선과 개정안들이 
많이 발현되어 사회적 약자가 보다 살기 좋은 제도적 안전 장치를 확고히 마련해야 하는 것에 목적을 두고 있음.
행동 패턴: 평일 저녁에 관련 뉴스 보도를 시청하고, 정치, 경제, 사회, 과학 분야 방면의 채널들을 다방면으로 시청하고 가끔씩 facebook이나 SNS에 본인 생각했던 사상이나 사회적 전반적인 문제에 대한 자신의 인식과 풀어가야 할 과제에 관련한 내용 글을 주기적으로 업데이트 함.
고충: 정치, 법리적 해설, 사회 규범, 규율, 제도적 장치에 관심은 가지고 있으나 전문 자료나 지식을 습득하기에는 해당 자료를 손 쉽게 찾기 어려움, 또한 법률적 지식부족으로 인한 취득한 정보의 질과 해당 법률정보의 신뢰성을 갖추기가 어려움.
지식 수준: 법률 관련 전공이 아니기에 지식은 4년제 대학이지만 해외 유학파여서 한국의 정세 상황에 대해서
이해 하기가 부족한 지식 수준으로서 상경계열 본과 대학을 졸업, 교육 관련 소프트웨어 사용에 익숙함
페르소나 분석의 장점
사용자 중심의 설계: 직접 사용자의 요구와 기대를 반영하고 그에 해당된 관련 제품이나 종합 서비스를 
설계할 수 있다고 생각함.
게획 결정에 아주 유용하다: 팀 내에서 사용자에 대한 공통된 이해를 바탕으로 일관된 의사 결정을 
내릴 수 있다. 
효율적인 커뮤니케이션: 여러 팀원들 간의 커뮤니케이션을 팀원들간 소통하게 하고, 주요 목표를 명확히 
이 룰 수 있다.
...
그래서 자바 공화국이라고 할 정도로.
마이그레이션: 소프트웨어나 운영체제를 새로운 버전으로 업그레이드 하거나 다른 플랫폼으로 옮기는 것을 말함. 
Java, (C#)Unity, C++, C, js, python, Ruby, php, kotlin 
롤업(library), 웹팩(webpack) BABEL(Cross-browsing)  
Dependences, devDependences, polyfill (인터섹션  옵저버 Es6에서 돌아가는 것을 Es5로 다운 그레이 호환되게 할 수 있는 devDependences )
...
사례조사 GPT에 학습시켜 통계를 내보도록 한다. 


- [x] 240529(수) 오늘 작업한 내용:
  
교수님 요청 내용 가상 인물을 만들어 한 인물 
페르소나: "채용 담당자 김현수"
기본 정보
이름: 김현수
직책: IT 기업 인사담당자
경력: 7년
배경: 인사 관리 및 채용에 대한 풍부한 경험을 가진 전문가로, 특히 기술 인재 채용에 강점을 가지고 있음.
목표
회사의 기술적 요구에 맞는 우수한 개발자를 채용하는 것.
회사 문화와 잘 맞는 인재를 찾아 조직의 생산성과 팀워크를 높이는 것.
문제점
기술 스택이 빠르게 변하고 있어 최신 기술에 능통한 개발자를 찾기 어려움.
개발자 채용 시 이력서와 면접만으로 후보자의 실제 실력을 평가하기가 어려움.
많은 지원자 중에서 가장 적합한 개발자를 효율적으로 선발해야 하는 부담.
필요사항
지원자의 기술적 능력과 실전 경험을 평가할 수 있는 신뢰할 만한 방법.
지원자가 팀에 잘 융화될 수 있는지 평가할 수 있는 방법.
기술적 트렌드와 최신 기술에 대한 이해를 바탕으로 후보자를 평가할 수 있는 지침.
페르소나 기법을 적용한 평가 분석 내용
1. 기술적 역량 평가
목표: 지원자의 기술 스택과 실전 능력을 정확히 평가.

코딩 테스트: 실시간으로 문제를 해결하는 능력을 평가하기 위한 코딩 테스트를 실시.
프로젝트 포트폴리오 리뷰: 지원자가 참여한 프로젝트와 그 역할을 구체적으로 확인.
기술 면접: 현업 개발자가 참여하여 깊이 있는 기술적 질문을 통해 후보자의 전문성을 평가.
2. 문화 적합성 평가
목표: 회사의 문화와 가치관에 부합하는 지원자를 선발.

성향 검사: 지원자의 성향과 성격을 파악할 수 있는 심리 검사 도구 활용.
팀 면접: 실제 팀원과의 면접을 통해 팀워크와 커뮤니케이션 능력 평가.
상황 대처 능력 평가: 가상의 시나리오를 제시하고, 지원자가 어떻게 문제를 해결하는지 평가.
3. 최신 기술 트렌드 이해 평가
목표: 최신 기술에 대한 이해도와 학습 능력을 평가.

기술 블로그 또는 GitHub 분석: 지원자가 운영하는 기술 블로그나 GitHub 활동을 통해 최신 기술에 대한 관심과 적용 능력 평가.
기술 세미나 및 컨퍼런스 참여 여부: 지원자가 최근 기술 세미나나 컨퍼런스에 얼마나 참여했는지 확인.
기술 퀴즈: 최신 기술 트렌드와 관련된 퀴즈를 통해 지원자의 지식 수준 평가.
4. 커뮤니케이션 및 문제 해결 능력 평가
목표: 지원자의 문제 해결 능력과 의사소통 능력을 평가.

케이스 스터디: 실제로 발생할 수 있는 문제를 제시하고, 지원자가 어떻게 접근하고 해결하는지 평가.
페어 프로그래밍: 다른 개발자와 함께 문제를 해결하는 과정을 통해 협업 능력과 소통 능력 평가.
결론
